...
| `ExcludedBlockCodes` | `["*barrelrack*", "*tunrack*"]` | Block codes the desk never scans, as `*` wildcard patterns matched against the full `domain:path` code. The escape hatch for modded storage the desk handles badly; defaults cover Food Shelves' barrel/tun racks. Set to `[]` to disable. |
